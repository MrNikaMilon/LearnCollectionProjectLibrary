...
public class LibraryServiceImpl implements LibraryServiceable {
    ...
    public void takeBook(Long userId, Long bookId) {
        ...
        for(Map.Entry<User, List<Book>> iterator : libraryMap.entrySet()){
            for(Book book : iterator.getValue()){
                if(iterator.getKey().getUserName().equals("Admin")){
                    if(book.getBookId().equals(bookId)){
                        takeBookList.add(book);
                        out.println("\tBook: \"" + book.getTitle() + "\" was be added.");
                        iterator.getValue().removeIf(removedElement -> removedElement.getTitle().equals(takeBookList.iterator().next().getTitle()));
                        break;
                    }
                } else {
                    out.println("\tAnother person has the book: " + book.getTitle());
                    break;
                }
            }
        }
    ...
    public void showMap(){
        Map<Book, User> showedMap=  libraryMap.entrySet().stream().flatMap(map -> {
            var key = map.getKey();
            var value = map.getValue();
            return value.stream().map(toMap -> Map.entry(toMap, key));
        }).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
        showedMap.forEach((key, value) -> out.println("\tUser: " + value.getUserName() + ", Book:" +  key.getTitle() + ", Book id: " + key.getBookId()));
    }
}
